...
from auxiliary_utils.mesh_management import generate_2D_mesh, generate_1D_mesh
...
def gen_uniform_1d_mesh_from_interval_and_resolution(domain, mesh_resolution):
    return np_linspace(domain[0], domain[1], (mesh_resolution+1))

#---------
#explicit 1D diffusion calculation
#---------
def genXi() -> float:
    return np_uniform(-1,1)
...
def diffuCoeff_innerSumArg(k: int,
    ...
    if withGenXi:
        xi = genXi()
        if returnGenXi:
            return sigma/(k**2 * np_pi**2)*np_cos(np_pi*k*x)*xi, xi
        else:
            return sigma/(k**2 * np_pi**2)*np_cos(np_pi*k*x)*xi
        ...
        return sigma/(k**2 * np_pi**2)*np_cos(np_pi*k*x)*passedXi
    else:
        return sigma/(k**2 * np_pi**2)*np_cos(np_pi*k*x)
def _diffuCoeff_sumFen(P: int, 
                    sigma: float, 
                    withGenXi: bool,
                    passedXiList: list[float]=None):
    if withGenXi:
        return lambda t: sum((sigma/(k**2*np_pi**2)) * np_cos(np_pi * k * t) * genXi() for k in range(1, P+1))
    elif passedXiList is not None:
        return lambda t: sum((sigma/(k**2*np_pi**2)) * np_cos(np_pi * k * t) * passedXiList[k-1] for k in range(1, P+1))
    else:
        return lambda t: sum((sigma/(k**2*np_pi**2)) * np_cos(np_pi * k * t) for k in range(1, P+1))
...
class model:
    def __init__(self, 
                ...
                mesh_1d_domain_interval: list = [0,1],
                t_end_cdr: float = 0.5,
        ...
        - mesh_num_of_steps (int): Number of intervals in the mesh for diffusion system.
        """
        ...
        if self.model_type in ["ishigami", 'ishigami_vect']:
            ...
            self.mean = mean
            self.std = std
            self.mesh_num_of_steps = mesh_num_of_steps
            self.mesh=np_linspace(0, 1, mesh_num_of_steps+1)
        ...
        # For Diffusion system
            ...
            self.mean = mean
            self.std = std
            self.mesh_1d_domain_interval = mesh_1d_domain_interval
            self.mesh_num_of_steps = mesh_num_of_steps
            ...
            if mesh_directory is None:
                ...
                generate_1D_mesh(domain_interval=mesh_1d_domain_interval,
                                 mesh_num_of_steps=self.mesh_num_of_steps,
            ...
            self.mesh_save_dir = 'data/mesh_data/cdr'
        ...
        for N_val, sobols in self.sobolVals_clos.items():
            ...
            for key, expected in expectedSobols.items():
                if key not in absDiffs:
                    absDiffs[key] = []
                if get_diffs:
                    absDiffs[key].append(np_abs(expected - sobols.get(key, 0)))
                else:
                    absDiffs[key].append(np_array(sobols.get(key, 0)))
    ...
    def sampleInput(self):
        ...
        if self.model_type in ["ishigami", 'ishigami_vect']:
            return np_uniform(-np_pi, np_pi, size=self.vectSize)
        elif "diffusion_1d" in self.model_type:
            return np_uniform(-1, 1, size=self.vectSize)
        elif self.model_type in ['toy_model_x1sqr_plus_x2sqr', 'toy_model_vect']:
            return np_uniform(-1, 1, size=self.vectSize)
        elif self.model_type in ['toy_1_vect', 'toy_2_vect']:
            return np_uniform(0, 1, size=self.vectSize)
    ...
    def set_uniform_1D_mesh(self, interval):
        ...
        self.mesh_coords = gen_uniform_1d_mesh_from_interval_and_resolution(domain=interval, mesh_resolution=self.mesh_num_of_steps)
    ...
    def _ishigami(self, x_1, x_2, x_3):
        ...
        return np_sin(x_1) + self.a * np_sin(x_2)*np_sin(x_2) + self.b * x_3*x_3*x_3*x_3 * np_sin(x_1)

    def toy_model_x1sqr_plus_x2sqr(self, inputVect):
        """..."""
        x_1, x_2 = inputVect
        if self.choose_y1:
            output = self.a*x_1*np_sin(np_pi*x_2)
        elif self.choose_y2:
            output = self.a*x_1*np_cos(np_pi*x_2)
        else:
            output = self.a*x_1*np_sin(np_pi*x_2)*self.a*x_1*np_sin(np_pi*x_2) + self.a*x_1*np_cos(np_pi*x_2)*self.a*x_1*np_cos(np_pi*x_2)   
        if self.indicator:
            return int(output <= self.constraintVal)
        else: 
            return output
    ...
    def get_difference_bw_alpha_explAndInMesh(self, coefficients):
        ...
        for idx, x in enumerate(self.mesh_coords):
            alpha_from_expl = gen1DDiffusionCoeff(x=x, withGenXi=False, explicitXiList=coefficients)
            alpha_from_expl_arr.append(alpha_from_expl)
            difference_bw_explAndInMesh += np_abs(alpha_from_expl-alpha_in_mesh[idx])
        return (difference_bw_explAndInMesh)
    
    def diffuFen(self, coefficients, scalarDiffuIdx=0, reset=False, indicesToEvalProjAt=np_array([0])):
        if reset:
            self.reset_model()
        set_log_active(False)
        v = self.v
    ...
    def diffuFen_expl(self, coefficients, scalarDiffuIdx=0.5):
        u_x, _ = explicit_1D_Diff_fen(x=scalarDiffuIdx, 
                                        P=self.P, 
                                        mu=self.mean, 
                                        sigma=self.std, 
                                        withGenXi=False,
